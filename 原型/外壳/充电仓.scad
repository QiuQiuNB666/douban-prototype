...
//           座舱 Ø24.0 且与转角圆弧同心（转角处只剩 1.5 mm 壁）、座舱底 Ø18.3 的圆（磁铁）
...
//
// 对不上的地方（别当成已定稿）：座舱深度见 dock_depth 的注释；接口在哪条边分不出来，见 port_side。
...
dock_d = 24.0; // .01
// 【存疑】座舱深度。公开资料给不出唯一解，两个条件打架，要拿卡尺量实物：
//   · 想对上官方的总高 14.95（= 8.12 + 豆子 13.45 − 深），要 6.62
//   · 但豆子侧键的下沿离豆体底面只有约 2.7，加磁吸片 2.3 = 离舱底 5.0；
//     舱深超过 5.0，侧键就会顶到 Ø24 的舱壁（装配检查 which="case" 会报约 19 mm³）
//   现在默认取 6.62（官方总高那一头），把矛盾摆在明处，不拿一个编出来的数把检查糊过去
dock_depth = 6.62; // .01
// 【专利】座舱底那个圆（磁铁）的直径
magnet_d = 18.3; // .1
// 【估】充电触点朝哪边：0 = 朝盒子中心（豆子的绑带朝向转角）
contact_dir = 0;
...
module cuts() {
    // 座舱
    translate([dock_c[0], dock_c[1], dock_z]) cylinder(d = dock_d, h = case_h, $fn = 120);
    // 座舱口倒角
    ...
    // 舱壁上两个触点窝，也嵌进去
    translate([dock_c[0], dock_c[1], 0]) for (s2 = [-1, 1]) rotate(180 + contact_dir + s2 * contact_a)
...
module charge_case() {
    ...
    translate([dock_c[0], dock_c[1], 0]) {
        ...
        for (s = [-1, 1]) rotate(180 + contact_dir + s * contact_a)
...
if (show_case_model) {
    charge_case();
    // 豆子：绑带朝转角外侧，磁吸片贴在座舱底
    if (show_bean_in_case) translate([dock_c[0], dock_c[1], dock_z + plate_t]) rotate(45) bean(gap = 0);
}
